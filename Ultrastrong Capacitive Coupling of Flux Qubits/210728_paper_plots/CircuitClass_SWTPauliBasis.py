...
class Circuit(object):
    ...
    def single_Hamiltonian(self, nodes, EC=1.0, EJ=1.0):
        ...
        # Go over all nodes combinations implementing the interactions between them.
        H_single = 4 * EC * sum(
            Cinv[i, j] * self._op_kron(len(nodes), self.n - (self.V[i - np.amin(nodes)] / (8 * EC)) * self.id, i - np.amin(nodes),
                                       self.n - (self.V[j - np.amin(nodes)] / (8 * EC)) * self.id, j - np.amin(nodes))
            for i in nodes
            for j in nodes
            if np.abs(Cinv[i, j])
        ) - EJ * sum(
    ...
    def SW_effective_Hamiltonian(self, ψb0, E, ψb):
