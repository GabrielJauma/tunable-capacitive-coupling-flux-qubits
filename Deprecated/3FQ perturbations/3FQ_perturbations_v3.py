...
ax[1].plot(V, Jy_harm * V, color='k', linestyle='--', label=r'$\eta$')
# ax[1].legend()
for i in range(1,4):
    ax[i-1].set_xlabel(r' $\tilde{\delta}_V$ ')
    ax[i-1].set_title(labels[i])
    ax[i-1].plot(V, J_pert_1[:,i], color = Bold_4.mpl_colors[1], marker='^', markevery=6,     linewidth=2)
    ax[i-1].plot(V, J_pert_2[:,i], color = Bold_4.mpl_colors[3], marker='s', markevery=(2,6), linewidth=2)
    ax[i-1].plot(V, J_SWT[:,i],    color = Bold_4.mpl_colors[2], marker='o', markevery=(4,6), linewidth=2) 
    ax[i-1].ticklabel_format(axis="y", style="sci", scilimits=(0,0))
